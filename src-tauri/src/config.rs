...
/// Proxy configuration loaded from ~/.usageboard/config.json
...
/// Config file path: ~/.usageboard/config.json
fn config_path() -> Option<PathBuf> {
    dirs::home_dir().map(|home| home.join(".usageboard").join("config.json"))
}
