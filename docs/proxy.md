...
eUsage can route provider and plugin HTTP requests through an optional proxy.

- Supported proxy types: `socks5://`, `http://`, `https://`
- Config file: `~/.eusage/config.json`
- Default: off
- UI: none

## Config File

Create `~/.eusage/config.json`:
...
- Restart eUsage after changing the file.
...
This applies to provider and plugin HTTP requests that go through eUsage's built-in HTTP client.
