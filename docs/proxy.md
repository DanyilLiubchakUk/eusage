...
UsageBoard can route provider and plugin HTTP requests through an optional proxy.

- Supported proxy types: `socks5://`, `http://`, `https://`
- Config file: `~/.usageboard/config.json`
- Default: off
- UI: none

## Config File

Create `~/.usageboard/config.json`:
...
- Restart UsageBoard after changing the file.
...
This applies to provider and plugin HTTP requests that go through UsageBoard's built-in HTTP client.
