...
Wire successful desktop provider probe results into team sync with redaction, source facts, 5-second debounce, 1-minute hard upload cap, memory-only retry, partial acceptance handling, invalid-token disconnect, and bounded quit flush.
...
- [ ] After no changes for the configured 5-second window, the full batch posts to the team usage endpoint.
- [ ] If provider updates keep arriving, the pending batch posts after the configured 1-minute hard cap.
