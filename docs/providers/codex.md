...
UsageBoard Codex plugin auth lookup order:
...
If file-based OAuth credentials are missing, invalid, or fail with an auth/session error during refresh or usage lookup, UsageBoard tries the macOS keychain fallback. Non-auth usage failures, such as server errors or invalid responses, are shown directly.
