...
1. **UsageBoard Keychain** (`UsageBoard-copilot`) — Token previously cached by the plugin
...
Once authenticated via gh CLI, the plugin caches the token in the UsageBoard keychain for faster access on subsequent probes.
