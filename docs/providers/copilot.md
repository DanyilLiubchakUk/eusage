...
1. **eUsage Keychain** (`eUsage-copilot`) — Token previously cached by the plugin
...
Once authenticated via gh CLI, the plugin caches the token in the eUsage keychain for faster access on subsequent probes.
