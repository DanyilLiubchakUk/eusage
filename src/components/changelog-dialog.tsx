    ...
    return parts.map((part, i) => {
      ...
      if (part.type === "pr") {
        return (
          <button
            key={i}
            onClick={() => openUrl(`https://github.com/DanyilLiubchakUk/eusage/pull/${part.content.slice(1)}`).catch(console.error)}
            className={linkClass}
          >
            {part.content}
          </button>
      ...
      if (part.type === "commit") {
        return (
          <button
            key={i}
            onClick={() => openUrl(`https://github.com/DanyilLiubchakUk/eusage/commit/${part.content}`).catch(console.error)}
