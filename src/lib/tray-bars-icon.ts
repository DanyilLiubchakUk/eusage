...
export const TRAY_ICON_LIGHT_SYSTEM_COLOR = "#254434"
export const TRAY_ICON_DARK_SYSTEM_COLOR = "#d8f7e2"
...
function setSvgColorAttribute(svg: string, color: string): string {
  return svg.replace(/<svg\b([^>]*)>/i, (match, attrs: string) => {
    if (/\scolor=/.test(attrs)) {
      return match.replace(/\scolor="[^"]*"/i, ` color="${color}"`)
    }
...
function tintSvgMarkup(svg: string, color: string): string {
  return setSvgColorAttribute(svg, color).replace(
    /\b(fill|stroke)="(?!none\b|transparent\b)[^"]*"/gi,
    `$1="${color}"`
  )
}

function tintProviderIconUrl(iconUrl: string, color: string): string {
  const trimmed = iconUrl.trim()
  const base64Prefix = "data:image/svg+xml;base64,"
  if (!trimmed.startsWith(base64Prefix) || typeof atob !== "function") return trimmed

  try {
    const svg = atob(trimmed.slice(base64Prefix.length))
    if (!svg.trimStart().startsWith("<svg")) return trimmed
...
export function makeTrayBarsSvg(args: {
  ...
  if (style === "provider") {
    ...
    const href = typeof providerIconUrl === "string" ? tintProviderIconUrl(providerIconUrl, foregroundColor) : ""
